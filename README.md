...
1.2 
Metasploitable содержит множество уязвимых служб. Примерный список:  
  FTP (vsftpd 2.3.4)  
  SSH (OpenSSH 4.7p1)  
  Telnet  
  SMTP (Postfix)  
...
Провёл сканирование в режимах SYN, FIN, Xmas и UDP. Все сеансы записал в Wireshark.


  Различия в трафике:  
  SYN-скан — отправляется SYN, ответ SYN-ACK или RST.  
  FIN и Xmas — отправляется FIN или особый набор флагов, при открытом порте сервер молчит, при закрытом — RST.  
  UDP — если порт закрыт, возвращается ICMP-пакет, если открыт — ответ UDP или тишина.  


Таким образом, каждый метод позволяет по-разному обходить системы обнаружения и определять состояние портов.
